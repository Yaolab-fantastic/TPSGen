...
\author{Zhoujie Fan, Haimeng Zhang, Xiaoyu Wang, Hao Meng, Hong Liang, Zheng Wang, Huazhu Fu, Ching-Yu Cheng, Shan Cong*, Xiaohui Yao*}
\date{}
...
ITAG3.2 provides gene boundaries rather than experimentally resolved transcription start sites (TSSs) for every gene. Accordingly, gene start was used as a coordinate proxy for TSS-centered extraction. The 165-bp interval was selected as the fixed input length for the promoter-generation and tissue-scoring models, balancing promoter-proximal sequence coverage against model complexity; it should not be interpreted as a universal biological promoter boundary. The thesis dataset used positive-strand genes to maintain a common orientation. When users prepare negative-strand genes for the released tool, the corresponding downstream genomic interval must be reverse-complemented so that every FASTA record is represented in the promoter-to-gene orientation. The resulting model input is one or more 165-bp sequences containing only A, C, G and T.
...
For the original TransVAE-MLP experiment, the 14,071 records were randomly divided at the sequence level into 11,257 training, 1,404 validation and 1,407 test records. The validation set was used for model selection and the test set for the reported prediction assessment. The split is recorded here as part of the original project experiment; the current repository does not add an independent external test set or claim that the retained result tables constitute independent biological validation.
...
High-evidence sequence segments were collected across promoter records and consolidated into recurrent short sequence patterns in the original analysis. The reported summary included CTATT, ACTAA, ATCCA and TTTAT with 239, 188, 184 and 121 retained instances, respectively. These counts refer to retained model-derived sequence instances, not to numbers of experimentally validated transcription-factor binding sites. The available project record does not preserve a complete executable specification of every threshold used in the historical enrichment and similarity-merging analysis; consequently, the counts are presented as original-project results rather than as a newly reproduced motif benchmark.
...
The corresponding four-panel summary is shown in Fig.~\ref{fig:supp-dnabert}.
...
The historical expression predictor used convolutional, recurrent and densely connected feature-extraction components followed by a scalar regression output and was optimized with mean-squared error before integration into preGAN. The released checkpoint and its training log are retained in the repository, but the training script in the original project refers to an external absolute-path CSV that is not included in this release. The repository therefore does not provide sufficient evidence to assign a definitive training-set size or to reconstruct the reported record-level partition. The previously circulated total and split counts are internally inconsistent and cannot be mapped to a complete, versioned CSV in the released tool; they are consequently not reported here as verified sample counts. The original model-comparison table reports a PCC of 0.72 for the DenseNet--LSTM predictor; this value is retained as a historical fruit-expression regression result, not as an independently reproducible benchmark from the current package.

Generator and critic learning rates were \(1\times10^{-4}\), the gradient-penalty coefficient was 10, batch size was 32 and training ran for 10,000 iterations. The critic was updated five times per generator update, and intermediate model and sequence outputs were recorded every 100 iterations. These are historical experiment settings rather than newly optimized package defaults.
...
A separate model comparison evaluated 4-, 5- and 6-mer frequency agreement. The thesis reports preGAN PCC values of 0.95, 0.93 and 0.89, compared with 0.92, 0.90 and 0.88 for TransVAE-MLP-generated sequences and 0.91, 0.88 and 0.86 for the comparison WGAN. These aggregate frequency correlations assess similarity of short-word distributions and do not establish that individual candidates are functional promoters.
...
\subsection{Relevance to the released tool}
The repository retains the preGAN model components, masked-template data handling, training entry point and checkpoint-backed candidate-generation interface. These components preserve the condition-pair semantics and permit software-level testing of the generator and critic. The current release does not, however, distribute a validated preGAN generator checkpoint that can be claimed to reproduce the quantitative results in Table~\ref{tab:pregan-results}. The retained DenseNet--LSTM resource is an expression-constraint model and is not itself a sequence-generator checkpoint.

Accordingly, the standard installed workflow uses the released package-native candidate generator unless a compatible preGAN generator checkpoint is supplied explicitly. With such a checkpoint, the preGAN workflow accepts one or more 165-bp masked tomato promoter templates, generates the requested number of candidates, writes candidate sequences in FASTA and CSV formats, scores them with the TransVAE-MLP route and records the design and scoring backends in a provenance manifest. Historical preGAN figures and values remain original-project evidence; successful execution of a smoke-test checkpoint demonstrates the software path but does not reproduce or validate those historical biological claims.
...
For current package outputs, tissue bias is assessed from the complete four-value score profile rather than from the target score alone. For a requested target tissue (t), TPSGen reports the target score (s_t), the target-bias margin
\[
\Delta_t=s_t-\max_{j\ne t}s_j,
\]
...
\end{verbatim}
The \texttt{--seed} value controls deterministic package-native candidate generation and is recorded in the manifest; it does not make a neural-network checkpoint retraining run deterministic by itself. Historical thesis figures and quantitative summaries are separate retained project resources and must not be interpreted as outputs from this small demonstration.

\subsection{Current package output versus retained project result}
Current package output demonstrates executable interfaces, file schemas and provenance recording on user-supplied tomato FASTA records. It should be interpreted as a new run of the released software, with the backend and checkpoint stated in its manifest. Retained thesis results document the original model experiments, including their original datasets, splits, training procedures and evaluation metrics. They are not silently substituted for outputs from the bundled demo.

This distinction is particularly important for the three mechanisms. The DNABERT thesis analyses include pretraining, promoter classification, attention-derived positional evidence, motif enrichment and mutagenesis; a fresh \texttt{predict-dnabert} run produces inference evidence, whereas the separate \texttt{annotate-dnabert} command processes precomputed attention resources. The preGAN thesis results were obtained from the historical paired training experiment, while the current package requires a compatible generator checkpoint for generation. The TransVAE-MLP checkpoint is available for four-target scoring, but the historical latent-space genetic-algorithm design procedure is not presented as a validated current API. These boundaries allow the supplement to report the original scientific results without overstating what a standard installation reproduces automatically.

\bibliographystyle{plain}\bibliography{application_note_references}
\end{document}
