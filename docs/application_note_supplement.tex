...
High-evidence sequence segments were collected across promoter records and consolidated into model-derived recurrent motif-like patterns, subsequently termed motifs for brevity. The resulting summary included CTATT, ACTAA, ATCCA and TTTAT with 239, 188, 184 and 121 retained instances, respectively. These counts refer to retained model-derived sequence instances, not to numbers of experimentally validated transcription-factor binding sites.
...
The model-development study also compared several prediction architectures on the same project dataset. The pure Transformer comparator reported 0.81, 0.85, 0.78 and 0.82, whereas TransVAE-MLP reported the values above. TransVAE-MLP was retained not because it was uniformly superior as a standalone predictor, but because its latent representation and four-target scoring interface support the integrated workflow. The Application Note therefore does not claim a new state-of-the-art expression predictor.
